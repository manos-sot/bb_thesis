...
//
//	double foo = custom_pwm(commandade2get);                                                   //auti 8a kalesei oses xreiazontai gia na 8esei to epi8umito frequency
	//UARTprintf("META TIN CUSTOM tbprd=%d \n",(int) *pTBPRD);
	//UARTprintf("META TIN CUSTOM reg2comp=%d \n", (int) foo);
	UARTprintf("test-reached here4\n");
/*       apo tin custom_pwm mas noiazoun 3 pointers:  a)pTBPRD [], b)pTBCLK [], c)pDUTY             */
// 	EHRPWMTimebaseClkConfig(SOC_EPWM_1_REGS, *pTBCLK, EHRPWM_INP_CLK_FREQ);
// 	UARTprintf("test-reached here5\n");
// 	EHRPWMPWMOpFreqSet(SOC_EPWM_1_REGS, (int) *pTBPRD, (int) commandade2get[0],
//                        commandade2get[2], EHRPWM_SHADOW_WRITE_ENABLE);
// 	UARTprintf("test-reached here6\n");
// //ta apo katw, isws, na prepei na mpoun se loupa?
// 	EHRPWMLoadCMPA(SOC_EPWM_1_REGS, (int) foo, EHRPWM_SHADOW_WRITE_ENABLE, EHRPWM_COMPA_LOAD_COUNT_EQUAL_ZERO_OR_PERIOD,
//                    EHRPWM_CMPCTL_OVERWR_SH_FL);
EHRPWMTimebaseClkConfig(SOC_EPWM_1_REGS, 71428572, EHRPWM_INP_CLK_FREQ);
EHRPWMPWMOpFreqSet(SOC_EPWM_1_REGS, 1428571, 50,
										 1, EHRPWM_SHADOW_WRITE_ENABLE);

//ta apo katw, isws, na prepei na mpoun se loupa?
EHRPWMLoadCMPA(SOC_EPWM_1_REGS, 1285714, EHRPWM_SHADOW_WRITE_ENABLE, EHRPWM_COMPA_LOAD_COUNT_EQUAL_ZERO_OR_PERIOD,
								 EHRPWM_CMPCTL_OVERWR_SH_FL);
